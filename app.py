...
st.sidebar.title("SisCL Malavazzi v4.3")
...
# ----------------- 4. ESTOQUE & NOTAS FISCAIS -----------------
    ...
    with t_est:
        with st.form("cadastro_estoque"):
            ...
            qm = col2.number_input("Estoque Mínimo (Alerta de Reposição)", min_value=1, value=5)
    ...
    with t_nf:
        with st.form("form_nf"):
            ...
            p_vinc = st.text_input("Vincular Paciente (Opcional)", placeholder="Ex: João da Silva")
...
# ----------------- 5. PEDIDOS AOS FORNECECORES -----------------
    ...
    with st.form("novo_pedido"):
        ...
        it_ped = col1.text_input("Especificações Técnicas do Item", placeholder="Ex: Acuvue Oasys Astigmatism")
...
# ----------------- 6. FRENTE DE VENDAS -----------------
    ...
    if not df_p.empty and not df_e.empty:
        ...
        filtro_venda = df_e[df_e['item'] == i_venda]['preco_venda']
        valor_item = float(filtro_venda.values[0]) if not filtro_venda.empty else 0.0
